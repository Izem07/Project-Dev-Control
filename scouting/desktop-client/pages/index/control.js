...
async function buildAndroidApk() {
    ...
    // Direct log console focus to android
    logsStore['android'] = [];
    switchLogView('android');
    document.getElementById('log-select').value = 'android'; // Wait, let's make sure android option exists in select

    // Dynamically append Android log option if not present
    const select = document.getElementById('log-select');
    let hasAndroid = false;
    for (let i = 0; i < select.options.length; i++) {
        if (select.options[i].value === 'android') hasAndroid = true;
    }
    if (!hasAndroid) {
        const opt = document.createElement('option');
        opt.value = 'android';
        opt.text = 'Android Compiler Logs';
        select.appendChild(opt);
        select.value = 'android';
    }

    const command = 'flutter build apk --no-tree-shake-icons';
    const cwd = '../android'; // Path to android directory relative to desktop-client
    ...
    if (!result.success) {
        alert(`Failed to start compilation: ${result.error}`);
